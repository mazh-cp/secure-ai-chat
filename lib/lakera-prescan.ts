...
export function detectCommonInjectionPatterns(content: string): {
  ...
  const patterns: Array<{
    ...
    {
      pattern: /forget\s+(previous|all|above|prior)\s+(instructions?|prompts?|rules?)/i,
    ...
    {
      pattern: /new\s+(instructions?|prompts?|rules?|directives?)/i,
    ...
    {
      pattern: /developer\s+mode|admin\s+mode|debug\s+mode/i,
    ...
    {
      pattern: /hypothetically|imagine\s+(that|if)/i,
    ...
    {
      pattern:
        /(show|reveal|display|print|output)\s+(your|the|system)\s+(prompt|instructions?|system\s+message)/i,
      name: 'Prompt Extraction',
      severity: 'high',
    },
    {
      pattern:
        /what\s+(are|were)\s+(your|the)\s+(initial|original|system)\s+(instructions?|prompts?)/i,
    ...
    {
      pattern: /rm\s+-rf|del\s+\/|format\s+c:/i,
