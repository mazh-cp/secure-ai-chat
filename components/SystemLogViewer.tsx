...
function getLevelBadge(level: SystemLogEntry['level']) {
  const badges = {
    info: (
      <span className="px-2 py-1 text-base rounded-full glass text-blue-300 border-blue-400/30">
        INFO
      </span>
    ),
    warning: (
      <span className="px-2 py-1 text-base rounded-full glass text-yellow-300 border-yellow-400/30">
        WARN
      </span>
    ),
    error: (
      <span className="px-2 py-1 text-base rounded-full glass text-red-300 border-red-400/50">
        ERROR
      </span>
    ),
    debug: (
      <span className="px-2 py-1 text-base rounded-full glass text-gray-300 border-gray-400/30">
        DEBUG
      </span>
    ),
  }
    ...
    return (
      <div className="p-12 text-center">
        ...
        <p className="text-theme-subtle text-base mt-1">
          System-level events and API failures will appear here
        </p>
      </div>
    )
  }
                ...
                <div className="space-y-1 text-base text-theme-muted">
                  {log.details.endpoint && (
                    <div>
                      ...
                      <code className="text-blue-300">{log.details.endpoint}</code>
                    </div>
                  )}
                  {log.details.method && (
                    <div>
                  ...
                  {log.details.statusCode && (
                    <div>
                      ...
                      <span
                        className={
                          log.details.statusCode >= 400 ? 'text-red-300' : 'text-green-300'
                        }
                      >
                        {log.details.statusCode}
                      </span>
                  ...
                  {log.details.duration !== undefined && (
                    <div>
                      <span className="font-medium">Duration:</span> {log.details.duration}ms
                    </div>
                  )}
                  {(() => {
                  ...
                  {log.details.requestHeaders &&
                    Object.keys(log.details.requestHeaders).length > 0 && (
                      <details className="mt-2">
                        <summary className="text-base text-theme-subtle cursor-pointer hover:text-theme">
                          Request Headers
                        </summary>
