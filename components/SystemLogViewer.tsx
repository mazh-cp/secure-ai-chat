...
function getLevelBadge(level: SystemLogEntry['level']) {
  const badges = {
    info: <span className="px-2 py-1 text-base rounded-full glass text-blue-300 border-blue-400/30">INFO</span>,
    warning: <span className="px-2 py-1 text-base rounded-full glass text-yellow-300 border-yellow-400/30">WARN</span>,
    error: <span className="px-2 py-1 text-base rounded-full glass text-red-300 border-red-400/50">ERROR</span>,
    debug: <span className="px-2 py-1 text-base rounded-full glass text-gray-300 border-gray-400/30">DEBUG</span>,
  }
  ...
  if (logs.length === 0) {
    return (
      <div className="p-12 text-center">
        ...
        <p className="text-theme-subtle text-base mt-1">System-level events and API failures will appear here</p>
      </div>
    )
  }
          ...
          >
            ...
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center space-x-3">
                <span className="text-3xl">{getServiceIcon(log.service)}</span>
                <div>
            ...
            {/* Message */}
            <div className="mb-3">
              <p className="text-base text-theme-muted">{log.message}</p>
            </div>

            {/* Details */}
            {log.details && (
              <div className="mt-3 p-3 glass-card rounded-xl border-blue-400/30">
                <p className="text-base text-blue-300 font-medium mb-2">Details:</p>
                <div className="space-y-1 text-base text-theme-muted">
                  ...
                  {log.details.requestHeaders && Object.keys(log.details.requestHeaders).length > 0 && (
                    <details className="mt-2">
                      <summary className="text-base text-theme-subtle cursor-pointer hover:text-theme">Request Headers</summary>
                      <pre className="mt-1 p-2 glass rounded text-base overflow-auto max-h-40 text-theme-muted font-mono">
            ...
            {log.metadata && Object.keys(log.metadata).length > 0 && (
              <details className="mt-2">
                <summary className="text-base text-theme-subtle cursor-pointer hover:text-theme">Metadata</summary>
                <pre className="mt-1 p-2 glass rounded text-base overflow-auto max-h-40 text-theme-muted font-mono">
