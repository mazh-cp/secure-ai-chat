...
function getActionBadge(action: LogEntry['action']) {
  const badges: Record<string, JSX.Element> = {
    request: <span className="px-2 py-1 text-base rounded-full glass text-brand-berry border-brand-berry/30" style={{ color: "var(--chip-text)" }}>Request</span>,
    allowed: <span className="px-2 py-1 text-base rounded-full glass text-brand-berry border-brand-berry/30" style={{ color: "var(--chip-text)" }}>✓ Allowed</span>,
    blocked: <span className="px-2 py-1 text-base rounded-full glass text-red-300 border-red-400/50" style={{ color: "var(--chip-text)" }}>🚫 Blocked</span>,
    scanned: <span className="px-2 py-1 text-base rounded-full glass text-brand-berry border-brand-berry/30" style={{ color: "var(--chip-text)" }}>Scanned</span>,
    error: <span className="px-2 py-1 text-base rounded-full glass text-yellow-300 border-yellow-400/30" style={{ color: "var(--chip-text)" }}>Error</span>,
    api_failure: <span className="px-2 py-1 text-base rounded-full glass text-red-300 border-red-400/50" style={{ color: "var(--chip-text)" }}>API Failure</span>,
    api_success: <span className="px-2 py-1 text-base rounded-full glass text-green-300 border-green-400/30" style={{ color: "var(--chip-text)" }}>API Success</span>,
    configuration: <span className="px-2 py-1 text-base rounded-full glass text-blue-300 border-blue-400/30" style={{ color: "var(--chip-text)" }}>Configuration</span>,
  }
  ...
  if (logs.length === 0) {
    return (
      <div className="p-12 text-center">
        ...
        <p className="text-theme-subtle text-base mt-1">Activity logs will appear here as you use the application</p>
      </div>
    )
  }
        ...
        {logs.map((log) => (
          ...
          >
            ...
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <h3 className="text-theme font-medium capitalize">
                    ...
                    {log.source && (
                      <span className="text-base text-theme-subtle">({log.source})</span>
                    )}
                  </div>
              ...
              {log.userIP && (
                <div className="text-base text-theme-muted glass px-2 py-1 rounded-lg" style={{ color: "var(--chip-text)" }}>
                  IP: {log.userIP}
                </div>
              )}
            </div>
                ...
                {(log.requestDetails.message || log.requestDetails.fileName) && (
                  <div className="space-y-2">
                    {log.requestDetails.message && (
                      <div>
                        <span className="text-base text-theme-subtle font-medium">Message:</span>
                        <p className="text-base text-theme-muted mt-1 break-words">
                    ...
                    {log.requestDetails.fileName && (
                      <div className="flex items-center space-x-4 flex-wrap">
                        <div>
                          <span className="text-base text-theme-subtle font-medium">File:</span>
                          <p className="text-base text-theme-muted">{log.requestDetails.fileName}</p>
                        </div>
                        {log.requestDetails.fileType && (
                          <div>
                            <span className="text-base text-theme-subtle font-medium">Type:</span>
                            <p className="text-base text-theme-muted">{log.requestDetails.fileType}</p>
                          </div>
                        )}
                        {log.requestDetails.fileSize && (
                          <div>
                            <span className="text-base text-theme-subtle font-medium">Size:</span>
            ...
            {log.lakeraDecision && (
              <div className={`p-3 rounded-xl glass-card ${
                ...
                <div className="flex items-center space-x-2 mb-2 flex-wrap">
                  ...
                  {log.lakeraDecision.flagged ? (
                    <span className="px-2 py-0.5 text-base rounded-full glass-card text-red-300 border-red-400/50">
                      Flagged
                    </span>
                  ) : (
                    <span className="px-2 py-0.5 text-base rounded-full glass-card text-green-300 border-green-400/30">
                      Safe
                    </span>
                  )}
                </div>
                ...
                {log.lakeraDecision.message && (
                  <p className="text-base text-theme-muted mb-2">{log.lakeraDecision.message}</p>
                )}

                {log.lakeraDecision.categories && Object.keys(log.lakeraDecision.categories).length > 0 && (
                  <div className="mt-2">
                    <p className="text-base text-theme-subtle mb-1">Categories:</p>
                      ...
                      {Object.entries(log.lakeraDecision.categories)
                        ...
                        .map(([category]) => {
                          ...
                          return (
                            <span
                              key={category}
                              className="px-2 py-1 text-base rounded-lg capitalize"
                              style={{
                ...
                {log.lakeraDecision.scores && Object.keys(log.lakeraDecision.scores).length > 0 && (
                  <div className="mt-2">
                    <p className="text-base text-theme-subtle">
                ...
                {log.lakeraDecision.payload && log.lakeraDecision.payload.length > 0 && (
                  <div className="mt-3 p-2 glass-card rounded-lg border-yellow-400/30">
                    <p className="text-base text-yellow-300 font-medium mb-2">Detected Threats ({log.lakeraDecision.payload.length}):</p>
                    <div className="space-y-2 max-h-48 overflow-y-auto">
                      {log.lakeraDecision.payload.map((item, idx) => (
                        <div key={idx} className="text-base bg-yellow-900/10 p-2 rounded border border-yellow-400/20">
                          ...
                          <div className="mt-1 text-theme-muted italic text-base">
                            &quot;{item.text.length > 80 ? item.text.substring(0, 80) + '...' : item.text}&quot;
                          </div>
                          {item.labels && item.labels.length > 0 && (
                            <div className="mt-1 flex flex-wrap gap-1">
                              {item.labels.map((label, labelIdx) => (
                                <span key={labelIdx} className="px-1.5 py-0.5 bg-yellow-800/20 rounded text-base text-yellow-200">
                                  {label}
                                </span>
                              ))}
                            </div>
                ...
                {log.lakeraDecision.breakdown && log.lakeraDecision.breakdown.length > 0 && (
                  <div className="mt-3 p-2 glass-card rounded-lg border-blue-400/30">
                    <p className="text-base text-blue-300 font-medium mb-2">
                    ...
                    <div className="space-y-1 max-h-40 overflow-y-auto">
                      {log.lakeraDecision.breakdown.map((detector, idx) => (
                        <div key={idx} className={`text-base p-1.5 rounded flex items-center justify-between ${
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-theme-subtle text-base">({detector.detector_id})</span>
                          </div>
                          {detector.detected && (
                            <span className="text-base text-red-300 font-medium">DETECTED</span>
                          )}
                        </div>
                      ))}
                    </div>
            ...
            {log.systemDetails && (
              <div className="mt-3 p-3 glass-card rounded-xl border-blue-400/30">
                <p className="text-base text-blue-300 font-medium mb-2">System Details:</p>
                <div className="space-y-1 text-base text-theme-muted">
                  ...
                  {log.systemDetails.stackTrace && (
                    <details className="mt-2">
                      <summary className="text-base text-theme-subtle cursor-pointer hover:text-theme">Stack Trace</summary>
                      <pre className="mt-1 p-2 glass rounded text-base overflow-auto max-h-40 text-red-300 font-mono">
                  ...
                  {log.systemDetails.responseBody && (
                    <details className="mt-2">
                      <summary className="text-base text-theme-subtle cursor-pointer hover:text-theme">Response Body</summary>
                      <pre className="mt-1 p-2 glass rounded text-base overflow-auto max-h-40 text-theme-muted font-mono">
            ...
            {log.error && (
              <div className="mt-3 p-3 glass-card rounded-xl border-red-400/30">
                <p className="text-base text-red-300 font-medium mb-1">Error:</p>
                <p className="text-base text-red-300">{log.error}</p>
              </div>
            )}

            {/* Success/Status */}
            <div className="mt-2 flex items-center">
              <span className={`text-base ${log.success ? 'text-green-300' : 'text-red-300'}`}>
      ...
      {logs.filter(log => 
        ...
        <div className="mt-6 p-4 glass-card rounded-xl border-red-400/50">
          <div className="flex items-center space-x-2 mb-4">
            ...
            <h2 className="text-xl font-semibold text-red-300">API Errors & Key Failures</h2>
            <span className="px-2 py-1 text-base rounded-full bg-red-500/20 text-red-300 border border-red-400/50">
              {logs.filter(log => 
                log.action === 'api_failure' || 
                log.action === 'error' || 
                (log.systemDetails?.statusCode && log.systemDetails.statusCode >= 400) ||
              ...
              .map((log) => (
                ...
                >
                  ...
                  <div className="mt-2 p-3 bg-red-900/20 rounded border border-red-400/30">
                    <p className="text-base font-medium text-red-200 mb-1">Full Error:</p>
                    <p className="text-base text-red-100 break-words whitespace-pre-wrap">
                      {log.error || 
                       (log.systemDetails?.responseBody ? 
                  ...
                  {(log.systemDetails?.statusCode === 401 || log.error?.includes('API key') || log.error?.includes('401')) && (
                    <div className="mt-2 p-2 bg-yellow-900/20 rounded border border-yellow-400/30">
                      <p className="text-base font-medium text-yellow-200 mb-1">🔑 Key Failure Detected:</p>
                      <ul className="text-base text-yellow-100 space-y-1 ml-4">
                  ...
                  {log.systemDetails && (
                    <details className="mt-2">
                      <summary className="text-base text-theme-subtle cursor-pointer hover:text-theme">
                        System Details
                      </summary>
                      <div className="mt-2 p-2 glass rounded text-base space-y-1">
                        {log.systemDetails.endpoint && (
                          <div><span className="font-medium">Endpoint:</span> {log.systemDetails.endpoint}</div>
                        )}
                        {log.systemDetails.method && (
                        ...
                        {log.systemDetails.responseBody && (
                          <details className="mt-2">
                            <summary className="text-base text-theme-subtle cursor-pointer">Response Body</summary>
                            <pre className="mt-1 p-2 glass rounded text-base overflow-auto max-h-40 text-theme-muted font-mono">
                              {typeof log.systemDetails.responseBody === 'string' 
                        ...
                        {log.systemDetails.stackTrace && (
                          <details className="mt-2">
                            <summary className="text-base text-theme-subtle cursor-pointer">Stack Trace</summary>
                            <pre className="mt-1 p-2 glass rounded text-base overflow-auto max-h-40 text-red-300 font-mono">
                  ...
                  {/* Request Details */}
                  {log.requestDetails && (
                    <div className="mt-2 p-2 glass rounded text-base">
