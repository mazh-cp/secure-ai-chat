...
function getScanStatusBadge(status: UploadedFile['scanStatus']) {
  switch (status) {
    case 'pending':
      return (
        <span
          className="px-2 py-1 text-sm rounded-full glass text-theme-muted"
    ...
    case 'scanning':
      return (
        <span
          className="px-2 py-1 text-sm rounded-full glass-button text-blue-300 animate-pulse"
          style={{ color: 'var(--chip-text)' }}
        >
          Scanning...
        </span>
    ...
    case 'safe':
      return (
        <span
          className="px-2 py-1 text-sm rounded-full glass-card text-green-300 border-green-400/30"
          style={{ color: 'var(--chip-text)' }}
        >
          ✓ Safe
        </span>
    ...
    case 'flagged':
      return (
        <span
          className="px-2 py-1 text-sm rounded-full glass-card text-red-300 border-red-400/50"
          style={{ color: 'var(--chip-text)' }}
        >
          ⚠️ Flagged
        </span>
    ...
    case 'error':
      return (
        <span
          className="px-2 py-1 text-sm rounded-full glass-card text-yellow-300 border-yellow-400/30"
          style={{ color: 'var(--chip-text)' }}
        >
          ⚠️ Error
        </span>
    ...
    case 'not_scanned':
      return (
        <span
          className="px-2 py-1 text-sm rounded-full glass text-theme-subtle border-brand-berry/20"
          style={{ color: 'var(--chip-text)' }}
        >
          Not Scanned
        </span>
...
export default function FileList({
  ...
  return (
    <div className="space-y-3">
      ...
      {files.map(file => (
        <div key={file.id} className="glass-card rounded-xl p-4 hover:scale-[1.02] transition-all">
            ...
            <div className="flex items-start space-x-3 flex-1 min-w-0">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center space-x-3 mt-1">
                  <span className="text-theme-muted text-sm">{formatFileSize(file.size)}</span>
                  <span className="text-theme-subtle">•</span>
                  <span className="text-theme-muted text-sm">{formatDate(file.uploadedAt)}</span>
                </div>
            ...
            <div className="flex items-center space-x-2 ml-4 flex-shrink-0">
              ...
              {!lakeraScanEnabled &&
                (file.scanStatus === 'pending' ||
                  file.scanStatus === 'error' ||
                  file.scanStatus === 'not_scanned') && (
                  <span className="text-sm text-yellow-400 ml-2">(Scanning disabled)</span>
                )}
            ...
            >
              ...
              {file.checkpointTeDetails && (
                <div className="space-y-2">
                  ...
                  >
                    {file.scanStatus === 'flagged'
                      ? '🛡️ Check Point TE Security Alert:'
                      : '🛡️ Check Point TE Analysis:'}
                  </p>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                      ...
                      <div>
                        ...
                        <span
                          className={`font-medium ${
                            file.checkpointTeDetails.logFields?.verdict === 'Malicious'
                              ? 'text-red-400'
                              : file.checkpointTeDetails.logFields?.verdict === 'Suspicious'
                                ? 'text-yellow-400'
                                : 'text-green-400'
                          }`}
                        >
                    ...
                    {file.checkpointTeDetails.logFields?.severity && (
                      <div>
                        ...
                        <span
                          className={`font-medium ${
                            file.checkpointTeDetails.logFields?.severity === 'Critical'
                              ? 'text-red-400'
                              : file.checkpointTeDetails.logFields?.severity === 'High'
                                ? 'text-orange-400'
                                : file.checkpointTeDetails.logFields?.severity === 'Medium'
                                  ? 'text-yellow-400'
                                  : 'text-blue-400'
                          }`}
                        >
                    ...
                    {file.checkpointTeDetails.logFields?.action && (
                      <div>
                        ...
                        <span className="text-theme font-medium capitalize">
                          {file.checkpointTeDetails.logFields?.action}
                        </span>
                      </div>
                    )}
                    ...
                    {file.checkpointTeDetails.logFields?.file_risk !== undefined && (
                      <div>
                        ...
                        <span
                          className={`font-medium ${
                            file.checkpointTeDetails.logFields?.file_risk >= 7
                              ? 'text-red-400'
                              : file.checkpointTeDetails.logFields?.file_risk >= 4
                                ? 'text-yellow-400'
                                : 'text-green-400'
                          }`}
                        >
                    ...
                    {file.checkpointTeDetails.logFields?.confidence !== undefined && (
                      <div>
                        ...
                        <span className="text-theme">
                          {Math.round((file.checkpointTeDetails.logFields?.confidence || 0) * 100)}%
                        </span>
                      </div>
                    )}
              ...
              {file.scanDetails && (
                <>
                  {file.scanDetails.categories &&
                    Object.keys(file.scanDetails.categories).length > 0 && (
                      <div className="mt-2 pt-2 border-t border-white/10">
                        <p className="text-theme-subtle text-sm font-medium mb-1">
                          Security Categories:
                        </p>
                    ...
                    <div className="mt-3 p-2 glass-card rounded-lg border-yellow-400/30">
                      ...
                      <div className="space-y-2 max-h-48 overflow-y-auto">
                        {file.scanDetails.payload.map((item, idx) => (
                          <div
                            key={idx}
                            className="text-sm bg-yellow-900/10 p-2 rounded border border-yellow-400/20"
                          >
                            ...
                            {item.labels && item.labels.length > 0 && (
                              <div className="mt-1 flex flex-wrap gap-1">
                                {item.labels.map((label, labelIdx) => (
                                  <span
                                    key={labelIdx}
                                    className="px-1.5 py-0.5 bg-yellow-800/20 rounded text-sm text-yellow-200"
                                  >
                                    {label}
                                  </span>
                                ))}
                  ...
                  {file.scanDetails.breakdown && file.scanDetails.breakdown.length > 0 && (
                    <div className="mt-3 p-2 glass-card rounded-lg border-blue-400/30">
                      <p className="text-sm text-blue-300 font-medium mb-2">
                        Detector Breakdown (
                        {file.scanDetails.breakdown.filter(d => d.detected).length}/
                        {file.scanDetails.breakdown.length} detected):
                      </p>
                      <div className="space-y-1 max-h-40 overflow-y-auto">
                        {file.scanDetails.breakdown.map((detector, idx) => (
                          <div
                            key={idx}
                            className={`text-sm p-1.5 rounded flex items-center justify-between ${
                              detector.detected
                          ...
                          >
                            <div className="flex items-center gap-2">
                              <span
                                className={detector.detected ? 'text-red-300' : 'text-green-300'}
                              >
                                {detector.detected ? '⚠️' : '✓'}
                              </span>
