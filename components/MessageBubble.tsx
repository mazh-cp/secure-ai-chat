      ...
      <div className={`max-w-[80%] ${isUser ? 'items-end' : 'items-start'} flex flex-col`}>
        <div
          className={`rounded-2xl px-4 py-3 ${
            isUser ? 'glass-button text-theme' : 'glass text-theme'
          } ${
            ...
            >
              {scanResult.flagged ? (
                    ...
                    <div className="mt-2 space-y-1">
                      ...
                      <div className="space-y-1 max-h-32 overflow-y-auto">
                        {scanResult.payload.map((item, idx) => (
                          <div
                            key={idx}
                            className="text-sm text-red-100/80 bg-red-900/20 p-1.5 rounded"
                          >
                            ...
                            {item.labels && item.labels.length > 0 && (
                              <div className="mt-0.5 flex flex-wrap gap-1">
                                {item.labels.map((label, labelIdx) => (
                                  <span
                                    key={labelIdx}
                                    className="px-1 py-0.5 bg-red-800/30 rounded text-sm"
                                  >
                                    {label}
                                  </span>
                                ))}
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-sm text-brand-berry/80">Scanned - Safe</span>
                </div>
              )}
            </div>
          )}

          <p className={`text-sm mt-2 ${isUser ? 'text-theme-muted' : 'text-theme-subtle'}`}>
