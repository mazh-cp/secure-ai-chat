#!/usr/bin/env bash
# Secure AI Chat — install/reinstall on an Ubuntu VM (from GitHub via curl)
#
# Always performs a full purge of any prior instance before installing:
#   - Stops + disables + removes the systemd unit
#   - Kills stray node processes
#   - Wipes the install directory (preserves .env.local and .secure-storage)
#   - Fresh clone, npm ci, build, systemd, UFW
#
...
#
# --- Recommended entry point for new Ubuntu 22.04+ / 24.04 VMs ---
#   curl -fsSL https://raw.githubusercontent.com/mazh-cp/secure-ai-chat/main/scripts/install-remote-vm-v1.1.sh | bash
#
# Overrides (vars after the pipe):
#   BRANCH=v1.1.13 INSTALL_DIR=/opt/secure-ai-chat bash
#
...
log_info "=== Secure AI Chat: Fresh install on Ubuntu VM ==="
log_info "Install directory: $INSTALL_DIR"
log_info "Branch:            $BRANCH"
log_info "Node version:      $NODE_VERSION"
if [ "$USE_BUILD_FRESH" = "1" ] || [ "$USE_BUILD_FRESH" = "true" ]; then
  log_info "Build:             npm run build:fresh"
fi
...
# --- Phase 2: Pre-install purge (always runs; clears any prior instance) ---
log_info "Phase 2: Purging any existing installation..."

# 2a. Stop and disable the systemd service
...
# 2b. Remove the unit file and reload systemd
if [ -f "/etc/systemd/system/${SERVICE_NAME}.service" ]; then
  log_info "  Removing systemd unit /etc/systemd/system/${SERVICE_NAME}.service..."
  sudo rm -f "/etc/systemd/system/${SERVICE_NAME}.service"
  sudo systemctl daemon-reload
fi

# 2c. Kill any stray node processes still holding the app directory or port
sudo pkill -u "$APP_USER" -f "node.*start-standalone" 2>/dev/null || true
sudo pkill -u "$APP_USER" -f "node.*next" 2>/dev/null || true
sleep 1

# 2d. Backup config and wipe install directory
_backup="/tmp/secure-ai-chat-reinstall-$(date +%Y%m%d-%H%M%S)"
if [ -d "$INSTALL_DIR" ] && { [ -d "$INSTALL_DIR/.git" ] || [ -f "$INSTALL_DIR/package.json" ]; }; then
  log_info "  Backing up config to $_backup and wiping $INSTALL_DIR..."
  mkdir -p "$_backup"
  ...
  log_success "  Wiped. Config saved to $_backup"
else
  log_info "  No existing install found at $INSTALL_DIR — nothing to wipe"
fi

# 2e. Create (or recreate) app user and install directory
...
# 2f. Restore config if we backed it up
if [ -f "$_backup/.env.local" ]; then
  sudo cp -a "$_backup/.env.local" "$INSTALL_DIR/"
  log_success "  .env.local restored from backup"
fi
if [ -d "$_backup/.secure-storage" ]; then
  sudo cp -a "$_backup/.secure-storage" "$INSTALL_DIR/"
  log_success "  .secure-storage restored from backup"
fi
sudo chown -R "$APP_USER:$APP_GROUP" "$INSTALL_DIR" 2>/dev/null || true

log_success "Phase 2 done: prior instance purged, directory ready"
