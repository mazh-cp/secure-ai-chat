...
USE_MAIN_FRESH="${USE_MAIN_FRESH:-0}"
USE_V3="${USE_V3:-0}"
USE_V2="${USE_V2:-1}"
if [ "${USE_MAIN_FRESH}" = "1" ] || [ "${USE_MAIN_FRESH}" = "true" ]; then
  SCRIPT_NAME="upgrade-remote-production-main-fresh.sh"
elif [ "${USE_V3}" = "1" ] || [ "${USE_V3}" = "true" ]; then
...
$(if { [ "${USE_MAIN_FRESH:-}" = "1" ] || [ "${USE_MAIN_FRESH:-}" = "true" ] || [ "${USE_V3}" = "1" ] || [ "${USE_V3}" = "true" ] || [ "${USE_V2}" = "1" ] || [ "${USE_V2}" = "true" ]; } && [ -n "${RUN_TYPECHECK:-}" ]; then printf '%s\n' "export RUN_TYPECHECK=$(printf '%q' "$RUN_TYPECHECK")"; fi)
$(if { [ "${USE_MAIN_FRESH:-}" = "1" ] || [ "${USE_MAIN_FRESH:-}" = "true" ] || [ "${USE_V3}" = "1" ] || [ "${USE_V3}" = "true" ] || [ "${USE_V2}" = "1" ] || [ "${USE_V2}" = "true" ]; } && [ -n "${HEALTH_RETRIES:-}" ]; then printf '%s\n' "export HEALTH_RETRIES=$(printf '%q' "$HEALTH_RETRIES")"; fi)
